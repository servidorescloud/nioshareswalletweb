...
class User {
	...
	function add($username, $password, $confirmPassword, $mail)

	{

		if (empty($username) || empty($password) || empty($confirmPassword) || empty($mail))
...

		{
			...
			return "Username must be between 4 and 30 characters";
     		...
     		return "Email address $mail is considered invalid.";
...

		{
...

			{
            	...
            	$query = $this->mysqli->query("INSERT INTO users (`date`, `ip`, `username`, `password`, `mail`,`supportpin`) VALUES (\"" . date("n/j/Y g:i a") . "\", \"". $_SERVER['REMOTE_ADDR'] . "\", \"" . $username ."\", \"" . $password . "\",  \"" . $mail . "\",\"". rand(10000,99999) . "\");");				
	...
	//	global $hide_ids;
                ...
                $qrcode=$this->getQRCodeGoogleUrl(urlencode(''.$fullname.' Wallet '.$short.''), $secret);
                $oneCode = $this->getCode($secret);
