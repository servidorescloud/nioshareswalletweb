...
if (!empty($_SESSION['user_session'])) {
  ...
  $withdrawalletuser = $_SESSION['authstatus'];
  $secretauthcode = $_SESSION['authsecret'];
}

$mysqli = new Mysqli($db_host, $db_user, $db_pass, $db_name);
if (!empty($_SESSION['user_session'])) {
    if(empty($_SESSION['token'])) {
        $_SESSION['token'] = sha1('@s%a$l£t#'.rand(0,3200000));
    }
    ...
    if (!empty($_POST['jsaction'])) {
        ...
        $json['balance'] = $jsonbal;
                ...
                if (empty(trim($_POST['oldpassword'])) || empty(trim($_POST['newpassword'])) || empty(trim($_POST['confirmpassword']))) {
                    $json['message'] = "You have to fill all the fields";
                } elseif ($_POST['token'] != $_SESSION['token']) {
                    $json['message'] = "Tokens do not match";
                    $_SESSION['token'] = sha1('@s%a$l£t#'.rand(0,3200000));
                    ...
                    $result = $user->updatePassword($user_session, $oldpassword, $newpassword, $confirmpassword);
                    if ($result === true) {
                        $json['success'] = true;
        ...
        if (!empty($_POST['action'])) {
            switch ($_POST['action']) {
               ...
               $suportkey = $_SESSION['user_supportpin'];
               echo "<script>alert('Support Key: $suportkey'); </script> ";
...
if(isset($_POST['username'])){
    if ($response!= null && $response->success) {
        ...
        // Code here to handle a successful verification
            ...
            switch ($_POST['action']) {
                case "login":
                $result = $user->logIn(addslashes(trim($_POST['username'])), addslashes(trim($_POST['password'])), addslashes(trim($_POST['auth'])));
                if (!is_array($result)) {
                    $error['type'] = "login";
                    $error['message'] = $result;
                } else {
                	...
                	$_SESSION['authsecret'] = $result['secret'];
                    $_SESSION['authstatus'] = $result['authused'];
                    header("Location: index.php");
                }
                break;
                case "register":
                $result = $user->add(addslashes(trim(strtolower($_POST['username']))), addslashes(trim($_POST['password'])), addslashes(trim($_POST['confirmPassword'])), addslashes(trim(strtolower($_POST['mail']))));
                if ($result!==true){
                    $error['type'] = "register";
                    $error['message'] = $result;
                } else {
            ...
            break;
              ...
              //echo "<script>alert('ERROR: Captcha!');</script>";
              }
